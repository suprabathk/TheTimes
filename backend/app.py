...
result = urlparse(
    "postgres://flask_i22d_user:XLFsNLAqUGI8CUAzUqrRAfjQ1dEojtEU@dpg-cg5csdfdvk4pls4d5e30-a.singapore-postgres.render.com/flask_i22d")
...
def createBlogsRelation():
    cursor.execute(
        "create table Userblogs(id serial PRIMARY KEY, title varchar(20) unique not null, author varchar(20) unique not null, body varchar(200) unique not null, created_at timestamptz NOT NULL DEFAULT now(), userID int, FOREIGN KEY (userID) REFERENCES users(id))")
...
def fetchBlog(id):
    cursor.execute(f"select * from Userblogs where id = {id}")
    item = cursor.fetchall()[0]
    data = {"id": item[0], "title": item[1], "author": item[2], "body": item[3], "created_at": item[4]}
    return data
...
def singlePost(id):
    ...
    if (request.method == "PUT"):
        try:
            return updateDB(request.json['title'], request.json['author'], request.json['body'], id)
        except Exception as e:
            print(e)
